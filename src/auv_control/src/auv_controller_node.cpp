...
class AUVController : public rclcpp::Node {
 ...
 private:
  ...
  void onOdom(const nav_msgs::msg::Odometry::SharedPtr msg) {
    ...
    // World-frame pose — parse first so we have roll/pitch/yaw for the rotation.
    ...
    // The p3d plugin (libgazebo_ros_p3d) reports the twist of the root link
    // relative to the world frame, NOT in the body frame.  Rotate explicitly
    // using R_bw (world→body) = the same Rwb matrix used in buoyancyBodyFrame().
    ...
    const double ox = msg->twist.twist.angular.x;
    const double oy = msg->twist.twist.angular.y;
    const double oz = msg->twist.twist.angular.z;

    x_(0)   =  cy*cp*vx  +  sy*cp*vy  -  sp*vz;
    x_(1)   =  (cy*sp*sr - sy*cr)*vx  +  (sy*sp*sr + cy*cr)*vy  +  cp*sr*vz;
    x_(2)   =  (cy*sp*cr + sy*sr)*vx  +  (sy*sp*cr - cy*sr)*vy  +  cp*cr*vz;
    p_roll_ =  cy*cp*ox  +  sy*cp*oy  -  sp*oz;
    x_(3)   =  (cy*sp*sr - sy*cr)*ox  +  (sy*sp*sr + cy*cr)*oy  +  cp*sr*oz;
    x_(4)   =  (cy*sp*cr + sy*sr)*ox  +  (sy*sp*cr - cy*sr)*oy  +  cp*cr*oz;
  }

  void onReference(const std_msgs::msg::Float64MultiArray::SharedPtr m) {
    std::lock_guard<std::mutex> lk(mtx_);
    // Ignore all-zero keepalive messages from the legacy reference_generator
    // so the trajectory outer loop stays active when u_ref ≈ 0.
    if (m->data.empty() || std::abs(m->data[0]) < 0.01) return;
    manual_ref_ = true;
  ...
  void updateReferenceFromTrajectory() {
    ...
    // dz = wp.z - pos_z_: positive when target is above → need positive w (upward in z-up frame).
