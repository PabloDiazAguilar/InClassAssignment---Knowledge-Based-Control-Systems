...
class ReferenceGenerator : public rclcpp::Node {
 public:
  ReferenceGenerator() : Node("reference_generator") {
    ...
    declare_parameter<double>("u0",                  0.0);   // cruise surge ref (0 = let trajectory loop run)
    declare_parameter<double>("yaw_amplitude",       0.0);
    declare_parameter<double>("yaw_period_s",        60.0);
    declare_parameter<double>("heave_amp",           0.0);
